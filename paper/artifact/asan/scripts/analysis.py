...
class AnalysisOneResults:
  ...
  def __init__(self, binary_path, binary_args, base_dir, is_asan = True, is_crash = False, round = 0):
    ...
    self.__fuzzer_list   = ['afl', 'aflpp', 'ffafl', 'ffapp']
...
import json
data = {}
data['prog_driver'] = ['catdoc', 'exiv2', 'flvmeta', 'lou_checktable', 'MP4Box', 'nasm', 'nm-new', 'tcpdump', 'tcpprep', 'tiff2pdf', 'gif2tga']
data['prog_args'] = {"nm-new" : "-C @@", "catdoc" : "@@", "exiv2" : "@@", "flvmeta" : "@@", "MP4Box" : "-diso @@",
                      "lou_checktable" : "@@", "tiff2pdf" : "@@", "nasm" : "-o nasm.out @@", "gif2tga" : "@@",
                      "tcpdump" : "-evnnnr @@", "tcpprep" : "--auto=bridge --pcap=@@ --cachefile=/dev/null"}
data['is_asan'] = True
data['is_crash'] = False
with open('asan.queue.json', 'w') as f:
  json.dump(data, f)

data['is_crash'] = True
with open('asan.crash.json', 'w') as f:
  json.dump(data, f)
