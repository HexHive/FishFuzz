...
export FF_DRIVER_NAME=exiv2
export SRC_DIR=/benchmark/exiv2-trunk

tar xzf /benchmark/source/exiv2-0.26-trunk.tar.gz -C /benchmark
...
export EXTRA_LDFLAGS="-ldl -lpthread -lrt -lm -lz -lexpat"
$CXX $ADDITIONAL_FUNC $BC_PATH$FF_DRIVER_NAME.final.bc -o $FF_DRIVER_NAME.fuzz $EXTRA_LDFLAGS $ASAN_LIBS
...
cd && rm -r $SRC_DIR/ && tar xzf /benchmark/source/exiv2-0.26-trunk.tar.gz -C /benchmark
...
export ASAN_LIBS=$(find `llvm-config --libdir` -name libclang_rt.asan-`uname -m`.a)
export EXTRA_LDFLAGS="-ldl -lpthread -lrt -lm -lz -lexpat"
$CXX $ADDITIONAL_FUNC $BC_PATH$FF_DRIVER_NAME.cov.bc -o $FF_DRIVER_NAME.fuzz $EXTRA_LDFLAGS $ASAN_LIBS
