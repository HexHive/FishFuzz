...
export FF_DRIVER_NAME=tcpprep
export SRC_DIR=/benchmark/tcpreplay-4.4.0

cp -r /benchmark/source/tcpreplay-4.4.0 /benchmark/tcpreplay-4.4.0
...
cd $SRC_DIR && ./autogen.sh && ./configure --disable-shared --disable-local-libopts && make -j$(nproc)
...
export EXTRA_LDFLAGS="-ldl -lpthread -lrt -lm -lpcap -lopts"
...
cd && rm -r $SRC_DIR/ && cp -r /benchmark/source/tcpreplay-4.4.0 /benchmark/tcpreplay-4.4.0
