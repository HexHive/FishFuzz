...
export FF_DRIVER_NAME=gif2tga
export SRC_DIR=/benchmark/ngiflib-0.4

tar xzf /benchmark/source/0.4.tar.gz -C /benchmark
...
export EXTRA_LDFLAGS="-ldl -lpthread -lrt -lm -lz"
...
cd && rm -r $SRC_DIR/ && tar xzf /benchmark/source/0.4.tar.gz -C /benchmark
...
# step 3, static distance map calculation
cp /Fish++/dyncfg/dis_calc /FishFuzz/dyncfg/ && opt -dot-callgraph $BC_PATH$FF_DRIVER_NAME.0.5.precodegen.bc && mv $BC_PATH$FF_DRIVER_NAME.0.5.precodegen.bc.callgraph.dot $TMP_DIR/dot-files/callgraph.dot
$PREFUZZ/scripts/gen_initial_distance.py $TMP_DIR
